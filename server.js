...
app.post('/api/checkout/create-prop-account', async (req, res) => {
     ...
     if (!crmAccountUuid || !challengeUuid) {
       ...
       // clicked "Continue to payment" twice, or came back to retry).
       // This is expected business behavior, not a system failure - return
       // a distinct error code so the frontend can show a helpful message
       // instead of a generic "something went wrong".
       if (
                mtError &&
                mtError.type === 'error://crm-api/bad-request' &&
                /pay the challenge fee for your existing account/i.test(mtError.detail || '')
              ) {
                console.warn('create-prop-account: existing unpaid account for', crmAccountUuid, challengeUuid);
                return res.status(409).json({ error: 'EXISTING_UNPAID_ACCOUNT' });
       }
